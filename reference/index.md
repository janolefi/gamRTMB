# Package index

## All functions

- [`edf()`](https://janolefi.github.io/gamRTMB/reference/edf.md) :
  Effective degrees of freedom per smooth
- [`fam()`](https://janolefi.github.io/gamRTMB/reference/fam.md)
  [`print(`*`<gamRTMB_family>`*`)`](https://janolefi.github.io/gamRTMB/reference/fam.md)
  : Build a family object
- [`families()`](https://janolefi.github.io/gamRTMB/reference/families.md)
  : Available families
- [`gamRTMB()`](https://janolefi.github.io/gamRTMB/reference/gamRTMB.md)
  [`nobs(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/gamRTMB.md)
  [`coef(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/gamRTMB.md)
  [`fitted(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/gamRTMB.md)
  : Fit a distributional GAM
- [`logLik(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/logLik.gamRTMB.md)
  : Log-likelihood, and hence AIC and BIC
- [`plot(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/plot.gamRTMB.md)
  : Plot a gamRTMB fit
- [`predict(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/predict.gamRTMB.md)
  : Predictions from a gamRTMB fit
- [`residuals(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/residuals.gamRTMB.md)
  : Randomised quantile (pseudo) residuals
- [`smooth.construct(`*`<spde.smooth.spec>`*`)`](https://janolefi.github.io/gamRTMB/reference/smooth.construct.spde.smooth.spec.md)
  [`Predict.matrix(`*`<spde.smooth>`*`)`](https://janolefi.github.io/gamRTMB/reference/smooth.construct.spde.smooth.spec.md)
  : Matern SPDE smooth
- [`summary(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/summary.gamRTMB.md)
  [`print(`*`<summary.gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/summary.gamRTMB.md)
  : Summarise a gamRTMB fit
- [`vcov(`*`<gamRTMB>`*`)`](https://janolefi.github.io/gamRTMB/reference/vcov.gamRTMB.md)
  : Covariance of the fixed-effect coefficients
