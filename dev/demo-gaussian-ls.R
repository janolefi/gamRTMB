...
demo_gaussian_ls <- function(n = 600, seed = 42, plot_file = NULL) {
  ...
  f3 <- function(x) -1 + 0.9 * cos(2 * pi * x)          # log sigma
  dat <- data.frame(x1 = runif(n), x2 = runif(n))
  dat$mu_true <- f1(dat$x1) + f2(dat$x2)
  dat$ls_true <- f3(dat$x1)
  dat$y <- rnorm(n, dat$mu_true, exp(dat$ls_true))

  cat("\n== 1. fit ================================================\n")
  t0 <- proc.time()[3]
  fit <- gamRTMB(y ~ list(mu = ~ s(x1, k = 12) + s(x2, k = 12),
                              sigma = ~ s(x1, k = 12)),
                     family = gaussian_ls(), data = dat,
  ...
  print(utils::head(data.frame(x1 = grid$x1,
                               mu = pr$fit$mu, mu.se = pr$se$mu,
                               log.sigma = pr$fit$sigma,
                               sigma = exp(pr$fit$sigma)), 3))
  ## in-sample accuracy against the truth
  ins <- predict(fit)
  cat(sprintf("  RMSE(mu_hat, mu_true)        = %.4f  (sd(y) = %.3f)\n",
              sqrt(mean((ins$mu - dat$mu_true)^2)), sd(dat$y)))
  cat(sprintf("  RMSE(log sigma_hat, truth)   = %.4f\n",
              sqrt(mean((ins$sigma - dat$ls_true)^2))))
  ...
  cat(sprintf("  cor(log sigma_gamRTMB, log sigma_mgcv)= %.5f\n",
              cor(ins$sigma, log(1 / gm[, 2]))))
  ...
  fit2 <- gamRTMB(y ~ list(mu = ~ s(x1, k = 12) + s(x2, k = 12),
                               sigma = ~ s(x1, k = 12) + s(x2, k = 12)),
                      family = gaussian_ls(), data = dat, method = "REML")
  print(fit2)
  print(edf(fit2))
  ...
  g2 <- mgcv::gam(list(y ~ s(x1, k = 12, id = 1) + s(x2, k = 12, id = 1), ~ 1),
                  family = mgcv::gaulss(), data = dat)
  fid <- gamRTMB(y ~ list(mu = ~ s(x1, k = 12, id = 1) + s(x2, k = 12, id = 1)),
                     family = gaussian_ls(), data = dat)
  cat(sprintf("  mu ~ s(x1, id=1) + s(x2, id=1):  gamRTMB edf %s | mgcv gaulss edf %s\n",
  ...
  cat(sprintf("  S.scale equal across the id group: %s\n",
              isTRUE(all.equal(fid$design$parts$mu$smooths[[1]]$sm$S.scale,
                               fid$design$parts$mu$smooths[[2]]$sm$S.scale))))
  ## (b) an id tying smooths on DIFFERENT distributional parameters, which a
  ## gam formula cannot express at all
  fx <- gamRTMB(y ~ list(mu = ~ s(x1, k = 12, id = "sh") + s(x2, k = 12),
                             sigma = ~ s(x1, k = 12, id = "sh")),
                    family = gaussian_ls(), data = dat)
  cat("  one smoothness shared between mu and sigma:\n")
  print(edf(fx))
  ...
  if (!is.null(plot_file)) {
    ...
    plot(g1$x1, tr$mu$fit[, 1], type = "l", lwd = 2, ylim = c(-2.2, 2.2),
         xlab = "x1", ylab = "s(x1)", main = "mu: s(x1)")
    graphics::polygon(c(g1$x1, rev(g1$x1)),
                      c(tr$mu$fit[, 1] + 2 * tr$mu$se[, 1],
                        rev(tr$mu$fit[, 1] - 2 * tr$mu$se[, 1])),
    ...
    plot(g2$x2, tr2$mu$fit[, 2], type = "l", lwd = 2, ylim = c(-2, 2),
         xlab = "x2", ylab = "s(x2)", main = "mu: s(x2)")
    graphics::polygon(c(g2$x2, rev(g2$x2)),
                      c(tr2$mu$fit[, 2] + 2 * tr2$mu$se[, 2],
                        rev(tr2$mu$fit[, 2] - 2 * tr2$mu$se[, 2])),
                      col = grDevices::adjustcolor("steelblue", 0.25), border = NA)
    graphics::lines(g2$x2, f2(g2$x2) - mean(f2(dat$x2)), col = 2, lty = 2, lwd = 2)

    plot(g1$x1, p1$fit$sigma, type = "l", lwd = 2, ylim = c(-2.5, 0.5),
         xlab = "x1", ylab = "log sigma", main = "sigma: log-scale linear predictor")
