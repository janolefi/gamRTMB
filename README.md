...
| Criterion | `REML` by default (coefficients integrated out by the same Laplace approximation); `qREML` — quasi-REML, the same criterion optimised by extended Fellner–Schall — when REML will not converge, which is what fits four-parameter families the other two cannot start; `ML` only to compare models whose unpenalized structure differs |
